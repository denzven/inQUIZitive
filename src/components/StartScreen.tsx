  ...
  const questionMarksDecor = (
    <>
      <div className="animate-pop-in-absolute" style={{ position: 'absolute', top: '21%', left: '8%', transform: 'translate(-50%, -50%)', zIndex: 0, animationDelay: '0.1s' }}>
        <div style={{ fontSize: 'clamp(20rem, 50vw, 25rem)', color: 'var(--color-secondary)', transform: 'rotate(20deg)', opacity: 0.8, fontWeight: 900 }}>?</div>
      </div>
      <div className="animate-pop-in-absolute" style={{ position: 'absolute', top: '42%', left: '97%', transform: 'translate(-50%, -50%)', zIndex: 0, animationDelay: '0.2s' }}>
        <div style={{ fontSize: 'clamp(20rem, 50vw, 25rem)', color: 'var(--color-accent)', transform: 'rotate(-20deg)', opacity: 0.8, fontWeight: 900 }}>?</div>
      </div>
      <div className="animate-pop-in-absolute" style={{ position: 'absolute', top: '92%', left: '7%', transform: 'translate(-50%, -50%)', zIndex: 0, animationDelay: '0.3s' }}>
        <div style={{ fontSize: 'clamp(20rem, 50vw, 25rem)', color: 'var(--color-accent)', transform: 'rotate(45deg)', opacity: 0.8, fontWeight: 900 }}>?</div>
      </div>
      <div className="animate-pop-in-absolute" style={{ position: 'absolute', top: '115%', left: '87%', transform: 'translate(-50%, -50%)', zIndex: 0, animationDelay: '0.4s' }}>
        <div style={{ fontSize: 'clamp(20rem, 50vw, 25rem)', color: 'var(--color-action)', transform: 'rotate(-15deg)', opacity: 0.8, fontWeight: 900 }}>?</div>
      </div>
    </>
  );
          ...
          {roundsData.map((round, idx) => {
            ...
            return (
              <button 
                ...
                style={{ 
                  textAlign: 'center', 
                  backgroundColor: isDisabled ? 'var(--color-primary-container)' : 'var(--color-primary)',
                  cursor: isDisabled ? 'not-allowed' : 'pointer',
                  opacity: isDisabled ? 0.7 : 1,
                  transform: isDisabled ? 'none' : undefined,
                  border: isFocused ? '3px solid var(--color-accent)' : undefined
                }}
                onMouseEnter={(e) => {
